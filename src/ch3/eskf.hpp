...
//    F.template block<3, 3>(6, 9) = -Mat3T::Identity() * dt;                        // theta 对 bg
    // TODO: 第三题，左乘误差状态的离散运动学方程
    Mat18T F = Mat18T::Identity();                                                          // 主对角线
    F.template block<3, 3>(0, 3) = Mat3T::Identity() * dt;                                  // p 对 v
    F.template block<3, 3>(3, 6) = SO3::hat(R_.matrix() * (ba_ - imu.acce_)) * dt;   // v 对 theta
    F.template block<3, 3>(3, 12) = -R_.matrix() * dt;                                      // v 对 ba
    F.template block<3, 3>(3, 15) = Mat3T::Identity() * dt;                                 // v 对 g
    F.template block<3, 3>(6, 9) = -R_.matrix() * dt;                                       // theta 对 bg

    // TODO: 第二题，把运动方程拆开
//    VecT delta_p = dx_.template block<3, 1>(0, 0);
//    VecT delta_v = dx_.template block<3, 1>(3, 0);
//    VecT delta_theta = dx_.template block<3, 1>(6, 0);
//    VecT delta_bg = dx_.template block<3, 1>(9, 0);
//    VecT delta_ba = dx_.template block<3, 1>(12, 0);
//    VecT delta_g = dx_.template block<3, 1>(15, 0);
//
//    dx_.template block<3, 1>(0, 0) = delta_p + delta_v * dt;
//    dx_.template block<3, 1>(3, 0) = delta_v - R_.matrix() * SO3::hat(imu.acce_ - ba_) * dt * delta_theta - R_.matrix() * dt * delta_ba + delta_g * dt;
//    dx_.template block<3, 1>(6, 0) = SO3::exp(-(imu.gyro_ - bg_) * dt).matrix() * delta_theta - delta_bg * dt;
